...
FIELDS = ['subject_id', 'hadm_id', 'micro_specimen_id', 'charttime', 'spec_type_desc', 'test_name', 'org_name', 'ab_name', 'interpretation']

# read CSV
try:
    with open("/Cancer_MIMIC-IV/cancer_hosp/microbiologyevents.csv", "r", newline='', encoding="utf-8") as csvfile:
        ...
        for line_no, row in enumerate(reader, start=2):
            try:
                inp = row[idx["hadm_id"]].strip().split('.')[0]
